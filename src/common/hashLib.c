...
CONFIGURATION
To use the generic hashing library, configure VxWorks with the
INCLUDE_HASH component.
...
int hashTblInit
    ...
    handleInit (&hashId->handle, handleTypeHashTbl); /* initialize handle */
...
* Possible errnos generated by this routine include:
* \is
* \i 'S_memLib_BLOCK_ERROR'
* The block of memory to kfree is not valid.
* \ie
*
* SEE ALSO: hashTblDestroy(), hashTblTerminate()
*/
...
* ERRNO: N/A
*
* SEE ALSO: hashTblDestroy(), hashTblDelete()
*/
...
/*******************************************************************************
*
* handleTerminate - terminate handle
*
* Terminate a handle.
*
* RETURNS: OK, or ERROR if handle could not be terminated.
*/

int handleTerminate
    (
    HANDLE_ID handleId        /* handle to terminate */
    )
    {

    handleId->magic = 0;

    handleId->type  = handleTypeUnknown;
...
* \ie
*
* SEE ALSO: hashTblDelete(), hashTblTerminate()
*/
...
int hashTblDestroy
    ...
    {
    if (HANDLE_VERIFY (&hashId->handle, handleTypeHashTbl) != OK)
    return (ERROR);             /* invalid hash id */

    handleTerminate (&hashId->handle);      /* terminate handle */
...
HASH_NODE * hashTblEach
    ...
    int  (* routine)    /* the routine to call for each hash node */
        (
    HASH_NODE *   pNode,
    int routineArg
    ),
    ...
    pNode = (HASH_NODE *)sllEach (&hashId->pHashTbl[ix],
                              routine,
                              routineArg);
