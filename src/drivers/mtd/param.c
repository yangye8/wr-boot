...
void display_param_tlb(void)
{
    ...
    printf("%-24s:\t   hex\t\t   integer\n", "name");
          /*mach_type               :  00000090               144*/
    printf("-------------------------------------------------------------\n");
    for (i = 0; i < num; i++) {
        printf("%-24s:\t%08lx\t%13lu\n", 
...
char *get_linux_cmd_line(void)
{
    ...
    if (strncmp(from, linux_cmd_magic, 8) != 0) {
        printf("Wrong magic: could not get linux command line\n");
        return NULL;
    }
...
{
    if (argc == 1) {
        printf("invalid 'params' command: too few arguments\n");
...
static void command_set(int argc, const char **argv)
{
    ...
    if (argc != 3) {
        printf("invalid 'param set' command: too few(many) arguments\n");
        return;
    }
    ...
    if (strncmp(name, "linux_cmd_line", strlen(name)) == 0) {
        ...
        if (!ret) {
            printf("Change linux command line to \"%s\"\n", 
                linux_cmd_line);
        } else {
            printf("Failed changing linux command line\n");
        }
        return;
    }
    ...
    if (ret) {
        printf("Can not parsing arguments\n");
    }

    pre_value = get_param_value(name, &ret);
    ret = set_param_value(name, value);
    if (ret) {
        printf("Error: can not change '%s' parameter value\n", name);
    }
    printf("Change '%s' value. 0x%08lx(%d) to 0x%08lx(%d)\n",
...
static void command_reset_tlb(int argc, const char **argv)
{
    if (get_default_param_tlb()) {
        printf("There are no default parameter table\n");
    }
...
static void command_save(int argc, const char **argv)
{
    ...
    if (save_priv_data_blk())
      printf("Could not save vivi private data\n");
    else printf("Saved vivi private data\n");
}
