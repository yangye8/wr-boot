...
cfi_init(void)
{
     ...
     */
    printf("S3C2440 flash: probing %d-bit flash bus\n", s3c2440_map.buswidth*8);
...
smc_insert(struct nand_chip *this) {
    ...
    if (!this->data_buf) {
        printf("Unable to allocate NAND data buffer for S3C2440.\n");
        this->data_buf = NULL;
        return -ENOMEM;
    }
...
smc_init(void)
{
    ...
    mymtd = kmalloc(sizeof(struct mtd_info) + sizeof(struct nand_chip));

    if (!mymtd) {
        printf("Unable to allocate S3C2440 NAND MTD device structure.\n");
        return -ENOMEM;
    }
