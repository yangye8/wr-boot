...
{
        int i;
        printf("  Feature/Command Support: %4.4X\n", extp->FeatureSupport);
        printf("     - Chip Erase:         %s\n", extp->FeatureSupport&1?"supported":"unsupported");
        printf("     - Suspend Erase:      %s\n", extp->FeatureSupport&2?"supported":"unsupported");
        printf("     - Suspend Program:    %s\n", extp->FeatureSupport&4?"supported":"unsupported");
        printf("     - Legacy Lock/Unlock: %s\n", extp->FeatureSupport&8?"supported":"unsupported");
        printf("     - Queued Erase:       %s\n", extp->FeatureSupport&16?"supported":"unsupported");
        printf("     - Instant block lock: %s\n", extp->FeatureSupport&32?"supported":"unsupported");
        printf("     - Protection Bits:    %s\n", extp->FeatureSupport&64?"supported":"unsupported");
        printf("     - Page-mode read:     %s\n", extp->FeatureSupport&128?"supported":"unsupported");
        printf("     - Synchronous read:   %s\n", extp->FeatureSupport&256?"supported":"unsupported");
        for (i=9; i<32; i++) {
                if (extp->FeatureSupport & (1<<i))
                        printf("     - Unknown Bit %X:      supported\n", i);
        }

        printf("  Supported functions after Suspend: %2.2X\n", extp->SuspendCmdSupport);
        printf("     - Program after Erase Suspend: %s\n", extp->SuspendCmdSupport&1?"supported":"unsupported");
        for (i=1; i<8; i++) {
                if (extp->SuspendCmdSupport & (1<<i))
                        printf("     - Unknown Bit %X:               supported\n", i);
        }

        printf("  Block Status Register Mask: %4.4X\n", extp->BlkStatusRegMask);
        printf("     - Lock Bit Active:      %s\n", extp->BlkStatusRegMask&1?"yes":"no");
        printf("     - Valid Bit Active:     %s\n", extp->BlkStatusRegMask&2?"yes":"no");
        for (i=2; i<16; i++) {
                if (extp->BlkStatusRegMask & (1<<i))
                        printf("     - Unknown Bit %X Active: yes\n",i);
        }

        printf("  Vcc Logic Supply Optimum Program/Erase Voltage: %d.%d V\n",
               extp->VccOptimal >> 8, extp->VccOptimal & 0xf);
        if (extp->VppOptimal)
                printf("  Vpp Programming Supply Optimum Program/Erase Voltage: %d.%d V\n",
...
struct mtd_info *cfi_cmdset_0001(struct map_info *map, int primary)
{
        ...
        if (cfi->cfi_mode == CFI_MODE_CFI) {
                ...
                //printf(" Intel/Sharp Extended Query Table at 0x%4.4X\n", adr);
                if (!adr)
                        return NULL;
                ...
                if (extp->MajorVersion != '1' ||  
                    (extp->MinorVersion < '0' || extp->MinorVersion > '2')) { 
                        printf("  Unknown IntelExt Extended Query "
                               "version %c.%c.\n",  extp->MajorVersion,
                               extp->MinorVersion);
                        kfree(extp);
                        return NULL;
...
static struct mtd_info *cfi_intelext_setup(struct map_info *map)
{
    ...
    mtd = kmalloc(sizeof(*mtd));

    if (!mtd) {
        printf("Failed to allocate memory for MTD device\n");
    ...
    mtd->eraseregions = kmalloc(sizeof(struct mtd_erase_region_info) 
                    * mtd->numeraseregions);
    if (!mtd->eraseregions) { 
        printf("Failed to allocate memory for MTD erase region info\n");
        kfree(cfi->cmdset_priv);
        return NULL;
    }
    ...
    if (offset != devsize) {
        /* Argh */
        printf("Sum of region (%lx) != total size of set of interleave chips (%lx)\n", offset, devsize);
    ...
    for (i = 0; i < mtd->numeraseregions; i++) {
        printf("%d: offset = 0x%x, size = 0x%x, blocks = %d\n", 
    ...
    if (cfi->cfiq->BufWriteTimeoutTyp) {
        printf("Using buffer write method\n");
        mtd->write = cfi_intelext_write_buffers;
    } else {
#else
    }
#endif
        printf("Using word write method\n");
                     ...
                     unsigned long adr, cfi_word datum)
{
    ...
    if (timeo <= 0) {
        ...
        printf("  Offset = 0x%08lx, datum = 0x%08lx, status = 0x%08lx\n",
            adr, datum, status);
    ...
    *retlen = 0;
    ...
    if (ofs & (CFIDEV_BUSWIDTH-1)) {
        ...
        if (cfi_buswidth_is_2()) {
            ...
            return -22; /* should never happen, but be safe */
        }
                           ...
                           unsigned long adr, const u_char *buf, int len)
{
    ...
    for (;;) {
        ...
        if (timeo < 0) {
            ...
            DISABLE_VPP(map);
            printf("Chip not ready for buffer write. Xstatus = 0x%llx, status = %llx\n", (__u64)status, (__u64)cfi_read(map, cmd_adr));
    ...
    for (z = 0; z < len; z += CFIDEV_BUSWIDTH) {
        if (cfi_buswidth_is_1()) {
            map->write8(map, *((__u8*)buf)+1, adr+z);
        } else if (cfi_buswidth_is_2()) {
            map->write16(map, *((__u16*)buf)+1, adr+z);
        } else if (cfi_buswidth_is_4()) {
            map->write32(map, *((__u32*)buf)+1, adr+z);
        } else if (cfi_buswidth_is_8()) {
            map->write64(map, *((__u64*)buf)+1, adr+z);
        } else {
            DISABLE_VPP(map);
            cfi_write(map, CMD(0xff), adr);
            return -22;
    ...
    for (;;) {
        ...
        if (timeo-- < 0) {
            printf("Waiting for chip to be ready timed out in bufwrite\n");
            cfi_write(map, CMD(0xff), adr);
            return -5;
        }
    }
                             ...
                             unsigned long adr, void *thunk)
{
    ...
    if ((timeo <= 0) || (status & CMD(0x3a))) {
        ...
        if (status != CMD(status & 0xff)) {
            ...
            printf("Status is not identical for all chips: 0x%llx, Mergint to give 0x%02x\n", (__u64)status, chipstatus);
        }
        ...
        if ((chipstatus & 0x30) == 0x30) {
            printf("Chip reports improper command sequence: status 0x%llx\n", (__u64)status);
            ret = -5;
        } else if (chipstatus & 0x02) {
            /* Protection bit set */
            printf("Protection bit set: status 0x%llx\n", (__u64)status);
            ret = -30;
        } else if (chipstatus & 0x8) {
            /* Voltage */
            printf("Chip reports voltage low on erase: status 0x%llx\n", (__u64)status);
            ret = -5;
        } else if (chipstatus & 0x20) {
            printf("Chip erase failed at 0x%08lx: status 0x%llx\n", adr, (__u64)status);
            ret = -5;
        }
    }
...
static int do_printlockstatus_oneblock(struct map_info *map, struct flchip *chip, unsigned long adr, void *thunk)
{
        ...
        printf("block status register for 0x%08lx is %x\n",
...
static int do_xxlock_oneblock(struct map_info *map, struct flchip *chip, unsigned long adr, void *thunk)
{
    ...
    for (;;) {
        ...
        if (timeo-- == 0) {
            cfi_write(map, CMD(0x70), adr);
                        printf("waiting for unlock to complete timed out. Xstatus = %llx, status = %llx.\n", (__u64)status, (__u64)cfi_read(map, adr));
            cfi_write(map, CMD(0xff), adr);
            DISABLE_VPP(map);
            return -5;
        }
