...
{
    memcpy((void *)addr, (const void *)&value, 8);
}
...
static inline u8 *flash_make_addr(struct flash_info *info, flash_sect_t sect,
        unsigned int offset)
{
//    printf("%s: portwidth-%d,sect-%d, offset-%d, chip-lsb-%d, info->start[sect]:0x%x\n",__func__,info->portwidth,sect,offset,info->chip_lsb,info->start[sect]);
