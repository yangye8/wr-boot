...
set_cpu_clk(unsigned long clk)
{
    ...
    if (clks->clock == 0) {
        printf("Can not find cpu clock table\n");
        return -2;
    }
...
change_memctl_regs(unsigned long clk) 
{
    ...
    if (memclks->clock == 0) {
        printf("Can not find mem  clock table\n");
        return -2;
    }
#if 0
...
print_cpu_info(void)
{
    ...
    printf("--------------------------------------------\n");
    printf("Processor clock: %d Hz\n", get_cpu_clk());
    printf("AHB bus clock  : %d Hz\n", get_bus_clk(GET_HCLK));
    printf("APB bus clock  : %d Hz\n", get_bus_clk(GET_PCLK));
    printf("\nRegister values\n");
    printf("MPLLCON: 0x%08lx", mpll);
    printf("  (MDIV: 0x%04x, PDIV: 0x%02x, SDIV: 0x%02x)\n",
        GET_MDIV(mpll), GET_PDIV(mpll), GET_SDIV(mpll));
    printf("CLKDIVN: 0x%08lx\n\n", CLKDIVN);
#endif
}
...
change_baudrate(unsigned long clock)
{
    ...
    if (ret) {
        printf("There is no 'baudrate' parameter\n");
        return;
    }
...
int change_sys_clks(unsigned long cpu_clk, unsigned long ratio)
{
    int ret = 0;
#if 0
    printf("CPU Clock Change From %d Mhz  To %d Mhz \n",
                get_cpu_clk()/1000000,cpu_clk);
    if (ratio > 7) {
        printf("Wrong Clock Ration %d \n",ratio);
    }
    else
    {
        CLKDIVN = ratio;
...
time_wait(unsigned int sec, int unit)
{
    ...
    if (clock_tick_rate == 0) {
        printf("Can not get a clock tick rate\n");
        return;
    }
...
{
    printf("Usage:\n");
    printf("cpu info\t\t\t-- Display cpu informatin\n");
    printf("cpu set <clock> <ratio>\t\t-- Change cpu clock and bus clock\n");
}
...
static void command_cpu(int argc, const char **argv)
{
    ...
    case 4:
        if (strncmp("set", argv[1], 3) == 0) {
            unsigned long clock = strtoul(argv[2], NULL, 0);
            unsigned long ratio = strtoul(argv[3], NULL, 0);
            int ret = 0;

            ret = change_sys_clks(clock, ratio);
            if (ret)
                printf("Failed. Can not change cpu clock\n");
            else
                printf("OK\n");
            break;
        }
