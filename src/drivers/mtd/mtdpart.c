 ...
 *
 * $Revision: 1.1.1.1 $
 * $Id: mtdpart.c,v 1.1.1.1 2004/02/04 06:22:25 laputa Exp $
 *
 *
...
{
    mtd_partition_t *parts = mtd_parts;
    int i, nb_parts = *(nb_mtd_parts);

    printf("Number of partitions: %d\n", nb_parts);
          /*name            :       offset          size            flag*/
    printf("%-16s:\toffset\t\tsize\t\tflag\n", "name");
    printf("------------------------------------------------------------\n");
    for(i = 0; i < nb_parts; i++) {
        printf("%-16s:\t0x%08lx\t0x%08lx\t%d\n", 
            parts->name, parts->offset, 
            parts->size, parts->flag);  
        parts++;
    }
}
...
static void command_add(int argc, const char **argv)
{
    ...
    if (strlen(argv[1]) >= MAX_PART_NAME) {
        printf("too long partition name\n");
    }
    ...
    printf("%s: offset = 0x%08lx, size = 0x%08lx, flag = %d\n",
        part.name, part.offset, part.size, part.flag);

    return;

error_parse_arg:
    printf("Can't parsing arguments\n");
    return;
}
...
{
    if (argc == 1) {
        printf("invalid 'part' command: too few arguments\n");
        command_help(0, NULL);
        return;
    }
