/* vivi/include/mtd.h: Common Flash Interface structures 
 *
 * Based on linux/include/linux/mtd/mtd.h
 *
 * See http://support.intel.com/design/flash/technote/index.htm
 *
 * $Id: cfi.h,v 1.1.1.1 2004/02/04 06:22:25 laputa Exp $
 */

#ifndef __VIVI_MTD_CFI_H__
#define __VIVI_MTD_CFI_H__
...
printf("base:0x%x, cmd_addr:0x%x,addr:0x%x \n",base,cmd_addr,addr);
    val = cfi_build_cmd(cmd, map, cfi);
printf("val:0x%x\n",val);
    if (prev_val){
        *prev_val = cfi_read(map, addr);
    printf("*prev_val:0x%x\n",*prev_val);
    }
