 ...
 * GPL'd
 * $Id: gen_probe.c,v 1.1.1.1 2004/02/04 06:22:25 laputa Exp $
 */
...
struct mtd_info *mtd_do_chip_probe(struct map_info *map, struct chip_probe *cp)
{
    ...
    printf("gen_probe: No supported Vendor Command Set found\n");
...
printf("%s/ %s %d\n",__FILE__,__FUNCTION__, __LINE__);
    ...
    if (!genprobe_new_chip(map, cp, &cfi)) {
        ...
        printf("%s: Found no %s device at location zero\n",
               cp->name, map->name);
        return NULL;
    }       
#endif
...
#else
        cp->probe_chip(map, 0xff000000, &chip[0], &cfi);
#endif
    /*
    ...
    retcfi = kmalloc(sizeof(struct cfi_private) + cfi.numchips * sizeof(struct flchip));

    if (!retcfi) {
        printf("%s: kmalloc failed for CFI private structure\n", map->name);
        kfree(cfi.cfiq);
        return NULL;
    }
                 ...
                 struct cfi_private *cfi)
{
    ...
    default:
        printf("genprobe_new_chip called with unsupported buswidth %d\n", map->buswidth);
        return 0;
    }
    return 0;
}
                          ...
                          int primary)
{
    ...
    printf("Support for command set %04X not present\n",
           type);
